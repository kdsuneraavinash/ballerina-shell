 ...
 * and executing it.
 */
public class ReEvalImprovedExecutor extends Executor<ReEvalImprovedState, ReEvalImprovedContext, ShellInvoker> {
    private static final String TEMPLATE_FILE = "template.reeval.ftl";
    private static final String GENERATED_FILE = "main.bal";
    private static final String RESERVED_UNDEFINED_VAR = "__undefined";

    @SuppressWarnings("unused")
    public ReEvalImprovedExecutor() {
        this(TEMPLATE_FILE, new ReEvalImprovedState(), new AsyncShellInvoker(GENERATED_FILE));
    }

    public ReEvalImprovedExecutor(String templateName, ReEvalImprovedState state, ShellInvoker invoker) {
        super(templateName, state, invoker);
    }

    @Override
    public ReEvalImprovedContext currentContext(Snippet newSnippet) {
        ...
        List<ReEvalImprovedContext.StatementExpression> statementsAndExpressions = new ArrayList<>();
        ReEvalImprovedContext.StatementExpression newStatementOrExpression =
                new ReEvalImprovedContext.StatementExpression(newSnippet);
        Map<String, VariableDeclarationSnippet> variables = new HashMap<>(state.variableDefinitions());
        String newVarName = RESERVED_UNDEFINED_VAR;
        ...
        for (Snippet snippet : state.statementsAndExpressions()) {
            if (snippet != newSnippet) {
                statementsAndExpressions.add(new ReEvalImprovedContext.StatementExpression(snippet));
            }
        }
        ...
        return new ReEvalImprovedContext(imports, moduleDeclarations,
    ...
    @Override
    public void onSuccess(Snippet newSnippet) throws ExecutorException {
